#!/usr/bin/env python3
"""
Run inference with the SegResNetV2 object detection model on CryoET tomograms.

Usage:
    cd /Users/akshaylakkur/Projects/QBI/ && \
    PYTORCH_MPS_HIGH_WATERMARK_RATIO=0.0 python backend/inference/run_inference.py \
        --checkpoint czii-weights/weights_best.ckpt \
        --data_root ~/Downloads/czii-cryo-et-object-identification \
        --study_name TS_5_4 \
        --split test \
        --device mps \
        --dtype float32 \
        ...
        --output predictions_TS_5_4.json
"""
...
def predict_volume(
    ...
    for tile_volume, tile_offsets in tqdm(loader, desc=f"Inference ({volume.shape})"):
...
def run_inference(
    ...
    """
    Run full inference pipeline: load model, load volume, predict, decode, save.

    Returns:
        Submission dict with keys: experiment, particle_type, score, x, y, z
    """
    # Resolve device
    ...
    print(f"Loading volume: {study_name} ({split})...")
    volume = load_volume(data_root, study_name, split=split)
    print(f"Volume shape: {volume.shape}")

    # Default score thresholds (from checkpoint metadata or sensible defaults)
    if score_thresholds is None:
        score_thresholds = [0.265, 0.290, 0.195, 0.150, 0.550, 0.3]

    print(f"Running inference with window={window_size}, tiles={tiles_per_dim}...")
    t0 = time.time()
    ...
    print("Decoding detections with NMS...")
    ...
    print(f"Found {result['num_detections']} particles")
...
def main():
    parser = argparse.ArgumentParser(description="CryoET Object Detection Inference")
    parser.add_argument("--checkpoint", required=True, help="Path to .ckpt checkpoint file")
    parser.add_argument("--data_root", required=True, help="Root directory of the CZII CryoET dataset")
    parser.add_argument("--study_name", required=True, help="Study name, e.g. TS_5_4")
